...
var AuthClass = /** @class */ (function() {
	...
	AuthClass.prototype.configure = function(config) {
		...
		var conf = Object.assign(
			{},
			this._config,
			core_1.Parser.parseMobilehubConfig(config).Auth,
			config
		);
		this._config = conf;
		var _a = this._config,
		...
		if (!this._config.storage) {
			...
			if (cookieStorage)
				this._storage = new amazon_cognito_identity_js_1.CookieStorage(
					cookieStorage
				);
			else {
	 ...
	 */
	AuthClass.prototype.signIn = function(
			...
			return __generator(this, function(_b) {
				switch (_b.label) {
					case 0:
						return [
							4 /*yield*/,
							this._getUserData(user, { bypassCache: true }),
						];
					case 1:
						userData = _b.sent();
						smsMfaSettings = null;
					...
					case 5:
						...
						if (currentMFAType === 'NOMFA') {
							...
							return [
								2 /*return*/,
								this.rejectAuthError(Auth_1.AuthErrorTypes.InvalidMFA),
							];
						}
					...
					case 6:
						...
						return [
							2 /*return*/,
							this.rejectAuthError(Auth_1.AuthErrorTypes.NoMFA),
						];
					case 7:
						that = this;
						return [
	 ...
	 */
	AuthClass.prototype.confirmSignIn = function(
		user,
		code,
		mfaType,
		clientMetadata
	...
	AuthClass.prototype.completeNewPassword = function(
		user,
		password,
		requiredAttributes,
		clientMetadata
	 ...
	 */
	AuthClass.prototype.sendCustomChallengeAnswer = function(
		user,
		challengeResponses,
		clientMetadata
	) {
	 ...
	 * @return {Promise}
	 **/
	AuthClass.prototype.updateUserAttributes = function(
		user,
		attributes,
		clientMetadata
	) {
							...
							user.getUserData(
								function(err, data) {
									...
									for (var i = 0; i < data.UserAttributes.length; i++) {
										...
										var userAttribute = new amazon_cognito_identity_js_1.CognitoUserAttribute(
											attribute
										);
	 ...
	 */
	AuthClass.prototype.verifyUserAttribute = function(
		user,
		attr,
		clientMetadata
	) {
	 ...
	 */
	AuthClass.prototype.changePassword = function(
		user,
		oldPassword,
		newPassword,
		clientMetadata
	 ...
	 */
	AuthClass.prototype.forgotPasswordSubmit = function(
		username,
		code,
		password,
		clientMetadata
	...
	AuthClass.prototype.federatedSignIn = function(
		providerOrOptions,
		response,
		user
	) {
			...
			return __generator(this, function(_a) {
				switch (_a.label) {
					case 0:
						...
						if (
							!(
								types_1.isFederatedSignInOptions(providerOrOptions) ||
					...
					case 1:
						...
						try {
							loggedInUser = JSON.stringify(
								JSON.parse(this._storage.getItem('aws-amplify-federatedInfo'))
									.user
							);
							if (loggedInUser) {
								logger.warn(
			...
			return __generator(this, function(_b) {
				switch (_b.label) {
					case 0:
						...
						currentUrl =
							URL ||
							(core_1.JS.browserOrNode().isBrowser ? window.location.href : '');
